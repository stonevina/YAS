(window.constructQZFL = function() {
	...
	(function() {
		...
		if (window.ActiveXObject) {
			ua.ie = 6;
			(window.XMLHttpRequest || agent.indexOf("MSIE 7.0") > -1)
					&& (ua.ie = 7);
			...
			ua.firefox = parseFloat(
					(r.exec(agent) || r.exec("Firefox/3.3"))[1], 10)
		} else if (!navigator.taintEnabled) {
			m = /AppleWebKit.(\d+\.\d+)/i.exec(agent);
			ua.webkit = m ? parseFloat(m[1], 10) : document.evaluate
					? document.querySelector ? 525 : 420
					: 419;
			if ((m = /Chrome.(\d+\.\d+)/i.exec(agent)) || window.chrome)
				ua.chrome = m ? parseFloat(m[1], 10) : "2.0";
			else if ((m = /Version.(\d+\.\d+)/i.exec(agent))
					|| window.safariHandler)
		...
		if (!(ua.macs = agent.indexOf("Mac OS X") > -1)) {
			ua.windows = (m = /Windows.+?(\d+\.\d+)/i.exec(agent), m
					&& parseFloat(m[1], 10));
		...
		!ua.iOS
				&& (m = /OS (\d+(?:_\d+)*) like Mac OS X/i.exec(agent), ua.iOS = m
						&& m[1] ? true : false)
	})();
	QZFL.object = {
		...
		each : function(obj, callback) {
			var value, i = 0, length = obj.length, isObj = length === undefined
					|| typeof obj == "function";
			if (isObj)
				for (var name in obj) {
				...
				for (value = obj[0]; i < length
						&& false !== callback.call(value, value, i, obj); value = obj[++i]);
			return obj
		},
		getType : function(obj) {
			return obj === null ? "null" : obj === undefined
					? "undefined"
	...
	(function(w) {
		...
		do
			try {
				cw.siDomain
						&& (QZFL.config.resourceDomain = cw.siDomain.replace(
								"http://", "").split("/")[0]);
				cw.imgcacheDomain
						&& (QZFL.config.domain = cw.imgcacheDomain.replace(
								"http://", "").split("/")[0])
			} catch (err) {
				break
			}
	...
	QZFL.config = QZFL.config || {};
	typeof QZFL.config.debugLevel == "undefined"
			&& (QZFL.config.debugLevel = 0);
	typeof QZFL.config.defaultDataCharacterSet == "undefined"
			&& (QZFL.config.defaultDataCharacterSet = "GB2312");
	typeof QZFL.config.DCCookieDomain == "undefined"
			&& (QZFL.config.DCCookieDomain = "qzone.qq.com");
	typeof QZFL.config.domainPrefix == "undefined"
			&& (QZFL.config.domainPrefix = "qq.com");
	typeof QZFL.config.domain == "undefined"
			&& (QZFL.config.domain = "qzs.qq.com");
	typeof QZFL.config.resourceDomain == "undefined"
			&& (QZFL.config.resourceDomain = "qzonestyle.gtimg.cn");
	QZFL.config.gbEncoderPath = "http://" + QZFL.config.domain
			+ "/qzone/v5/toolpages/";
	QZFL.config.FSHelperPage = "http://" + QZFL.config.domain
			+ "/qzone/v5/toolpages/fp_gbk.html";
	QZFL.config.defaultShareObject = "http://" + QZFL.config.resourceDomain
			+ "/qzone/v5/toolpages/getset.swf";
	QZFL.config.staticServer = "http://" + QZFL.config.resourceDomain
			+ "/ac/qzone/qzfl/lc/";
	QZFL.css = {
		classFileNameCache : {},
		convertHexColor : function(color) {
			...
			return color.length == 6 ? [parseInt(color.substr(0, 2), 16),
					parseInt(color.substr(2, 2), 16),
					parseInt(color.substr(4, 2), 16)] : [0, 0, 0]
		},
		rgb2hsl : function(r, g, b) {
			var t, red = Math.max(r / 255, 0), green = Math.max(g / 255, 0), blue = Math
					.max(b / 255, 0), max = Math.max(red, green, blue), min = Math
					.min(red, green, blue), result = {
				h : 0,
				s : 0,
			...
			if (max != min) {
				if (max == red)
					result.h = (t = 60 * ((green - blue) / (max - min))) < 0
							? t + 360
							: t;
				else if (max == green)
		...
		getStyleSheetById : function(id) {
			var s;
			return (s = QZFL.dom.get(id)) && s.sheet
					|| (s = document.styleSheets) && s[id]
		},
		getRulesBySheet : function(sheetId) {
			var ss = typeof sheetId == "object" ? sheetId : QZFL.css
					.getStyleSheetById(sheetId), rs = {}, head, base;
		...
		getRuleBySelector : function(sheetId, selector) {
			...
			var _ss = QZFL.css.getStyleSheetById(sheetId), _rs = QZFL.css
					.getRulesBySheet(_ss);
		...
		insertCSSLink : function(url, opts) {
			...
			cssLink = (t = doc.getElementById(sid)) && t.nodeName == "LINK"
					? t
					: null;
			if (!cssLink) {
		...
		updateClassName : function(elem, removeNames, addNames) {
			...
			if (removeNames && typeof removeNames == "string" || addNames
					&& typeof addNames == "string") {
				if (removeNames == "*")
					oriName = "";
				else {
		...
		hasClassName : function(elem, name) {
			return elem && name ? elem.classList
					? elem.classList.contains(name)
					: name
							&& (" " + elem.className + " ").indexOf(" " + name
									+ " ") > -1 : false
		},
		addClassName : function(elem, names) {
			var _s = QZFL.css;
			return names
					&& (elem && elem.classList && !_s._reClassToken.test(names)
							? elem.classList.add(names)
							: _s.updateClassName(elem, null, names))
		},
	...
	QZFL.dom = {
		...
		removeElement : function(elem) {
			if (elem = QZFL.dom.get(elem)) {
				if (QZFL.userAgent.ie > 8 && elem.tagName == "SCRIPT")
					elem.src = "";
				elem.removeNode ? elem.removeNode(true) : elem.parentNode
		...
		getElementsByClassName : function(className, tagName, context) {
			return QZFL.selector((tagName || "") + "." + className, QZFL.dom
							.get(context))
		},
		...
		getFirstChild : function(elem) {
			...
			return elem.firstElementChild
					|| QZFL.dom.searchChain(elem && elem.firstChild,
		...
		getLastChild : function(elem) {
			...
			return elem.lastElementChild
					|| QZFL.dom.searchChain(elem && elem.lastChild,
		...
		getNextSibling : function(elem) {
			...
			return elem.nextElementSibling
					|| QZFL.dom.searchChain(elem && elem.nextSibling,
							"nextSibling", function(el) {
								return el.nodeType == 1
							})
		},
		getPreviousSibling : function(elem) {
			...
			return elem.previousElementSibling
					|| QZFL.dom.searchChain(elem && elem.previousSibling,
							"previousSibling", function(el) {
								return el.nodeType == 1
							})
		},
		...
		createElementIn : function(tagName, elem, insertFirst, attrs) {
			var _e = (elem = QZFL.dom.get(elem) || document.body).ownerDocument
		...
		getStyle : function(el, property) {
			...
			var w3cMode = document.defaultView
					&& document.defaultView.getComputedStyle, computed = !w3cMode
					? null
		...
		convertStyle : function(el, prop, value) {
			...
			value += typeof value === "number" && !rexclude.test(prop)
					? "px"
					: "";
			return {
				"prop" : prop,
		...
		setXY : function(elem, x, y) {
			var _ml = parseInt(QZFL.dom.getStyle(elem, "marginLeft")) || 0, _mt = parseInt(QZFL.dom
					.getStyle(elem, "marginTop"))
					|| 0;
		...
		getScrollLeft : function(doc) {
			...
			return _doc.defaultView
					&& _doc.defaultView.pageXOffset
					|| Math.max(_doc.documentElement.scrollLeft,
							_doc.body.scrollLeft)
		},
		getScrollTop : function(doc) {
			var _doc = doc || document;
			return _doc.defaultView
					&& _doc.defaultView.pageYOffset
					|| Math.max(_doc.documentElement.scrollTop,
							_doc.body.scrollTop)
		},
		...
		getScrollWidth : function(doc) {
			var _doc = doc || document;
			return Math.max(_doc.documentElement.scrollWidth,
					_doc.body.scrollWidth)
		},
		setScrollLeft : function(value, doc) {
			var _doc = doc || document;
			_doc[_doc.compatMode == "CSS1Compat" && !QZFL.userAgent.webkit
					? "documentElement"
		...
		setSize : function(el, w, h) {
			...
			QZFL.dom.setStyle(el, "width", (m = _r.exec(w)) ? m[1]
							? w
							: parseInt(w, 10) + "px" : "auto");
			QZFL.dom.setStyle(el, "height", (m = _r.exec(h)) ? m[1]
							? h
							: parseInt(h, 10) + "px" : "auto")
		},
		...
		getElementByTagNameBubble : function(elem, tn) {
			...
			elem = QZFL.dom.searchChain(elem = QZFL.dom.get(elem),
					"parentNode", function(el) {
						return el.tagName == tn || el.tagName == "BODY"
								|| --maxLv < 0
					});
			return !elem || maxLv < 0 ? null : elem
		},
		insertAdjacent : function(elem, where, html, isText) {
			var range, pos = ["beforeBegin", "afterBegin", "beforeEnd",
					"afterEnd"], doc;
			if (QZFL.lang.isElement(elem) && pos[where]
					&& (QZFL.lang.isString(html) || QZFL.lang.isElement(html))) {
				if (elem.insertAdjacentHTML && elem.insertAdjacentElement
						&& elem.insertAdjacentText)
					elem["insertAdjacent"
							+ (typeof html == "object" ? "Element" : isText
									? "Text"
									: "HTML")](pos[where], html);
				else {
					range = (doc = elem.ownerDocument).createRange();
					range[where == 1 || where == 2
							? "selectNodeContents"
							: "selectNode"](elem);
					range.collapse(where < 2);
					range.insertNode(typeof html != "string" ? html : isText
							? doc.createTextNode(html)
	...
	QZFL.event = {
		...
		addEvent : function(obj, eventType, fn, argArray) {
			...
			if (QZFL.userAgent.isiPad
				...
				efn = function(evt) {
					...
					if (t < 700) {
						...
						return fn
								&& fn.apply(obj, !argArray ? [QZFL.event
												.getEvent(evt)] : [QZFL.event
												.getEvent(evt)]
												.concat(argArray))
					}
					return true
			...
			cfn = cfn || function(evt) {
				return fn.apply(obj, !argArray
								...
								: [QZFL.event.getEvent(evt)].concat(argArray))
			};
		...
		trigger : function(obj, eventType) {
			var l = obj && QZFL.event._eventListDictionary[obj.eventsListUID], handlers = l
					&& l[eventType] && l[eventType].handlers, i;
			if (handlers)
				try {
					for (i in handlers)
		...
		getEvent : function(evt) {
			...
			if (!evt) {
				...
				while (c && ct < _s.MAX_LEVEL) {
					if (c.arguments && (evt = c.arguments[0])
							&& typeof evt.button != "undefined"
							&& typeof evt.ctrlKey != "undefined")
						break;
					++ct;
					c = c.caller
				}
		...
		_bindReady : function() {
			...
			if (document.addEventListener) {
				...
				document
						.attachEvent("onreadystatechange", QZFL.event._domReady);
		...
		delegate : function(delegateDom, selector, eventType, fn, argsArray) {
			...
			QZFL.imports(path, function() {
						QZFL.event.delegate(delegateDom, selector, eventType,
								fn, argsArray)
					})
		},
	...
	QZFL.queue = function() {
		...
		var _extend = {
			...
			timedChunk : function(key, conf) {
				...
				if (_q) {
					_conf = QZFL.lang.propertieCopy(conf, QZFL.queue._tcCof,
							null, true);
					setTimeout(function() {
								var _start = +new Date;
								do
									QZFL.queue.shift(key);
								while (QZFL.queue.getLen(key) > 0
										&& +new Date - _start < _conf.runTime);
			...
			_exec : function(value, key, source) {
				...
				try {
					...
					QZFL.console.print("QZFL Queue Got An Error: [" + e.name
									+ "]  " + e.message, 1)
				}
			}
		};
	...
	QZFL.string = {
		...
		trim : function(str) {
			return QZFL.string.commonReplace(str + "",
		...
		cut : function(str, bitLen, tails) {
			...
			if (isNaN(bitLen))
				return str;
			var len = str.length, i = Math.min(Math.floor(bitLen / 2), len), cnt = QZFL.string
					.getRealLen(str.slice(0, i));
			for (; i < len && cnt < bitLen; i++)
				cnt += 1 + (str.charCodeAt(i) > 255);
			return str.slice(0, cnt > bitLen ? i - 1 : i)
					+ (i < len ? tails : "")
		},
		...
		URI : function(s) {
			...
			if (s.indexOf("://") < 1)
				s = location.protocol
						+ "//"
						+ location.host
						+ (s.indexOf("/") == 0 ? "" : location.pathname.substr(
								0, location.pathname.lastIndexOf("/") + 1)) + s;
			var depart = s.split("://");
			if (QZFL.object.getType(depart) == "array" && depart.length > 1
					&& /^[a-zA-Z]+$/.test(depart[0])) {
				...
				if (QZFL.object.getType(h) == "array") {
					...
					this.href = s;
					var se = depart[1].lastIndexOf("?"), ha = depart[1]
							.lastIndexOf("#");
	...
	QZFL.lang = {
		...
		isNode : function(o) {
			return o
					&& (typeof o.nodeName != "undefined" || typeof o.nodeType != "undefined")
		},
	...
	(function() {
		...
		QZFL.object.extend(QZFL.util, {
			...
			evalGlobal : function(js) {
				js = String(js);
				var obj = document.createElement("script"), head = document.documentElement
			...
			genHttpParamString : function(o) {
				return QZFL.util.commonDictionaryJoin(o, null, null, null,
				...
				if (l && l.length)
					for (var i = 0, len = l.length; i < len; ++i) {
						...
						if (ks.length > 1) {
							...
							vs = t.split(vq);
							res[ks[0]] = vs.slice(vq.length,
									vs.length - vq.length).join(vq)
						} else
							ks[0] && (res[ks[0]] = true)
					}
				return res
	...
	QZFL.lang.obj2str = function(obj) {
		...
		if (typeof obj == "object") {
			...
			t = [];
			for (var i in obj)
				t.push((sw ? "" : '"' + QZFL.string.escString(i) + '":')
						+ obj2str(obj[i]));
			t = t.join();
			return sw ? "[" + t + "]" : "{" + t + "}"
	...
	QZFL.lang.propertieCopy = function(s, b, propertiSet, notOverWrite) {
		var l = !propertiSet || typeof propertiSet != "object"
				? b
				: propertiSet;
		s = s || {};
		for (var p in l)
	...
	QZFL.pageEvents = function() {
		function _ihp() {
			var qs = location.search.substring(1), qh = location.hash
					.substring(1), s, h, n;
		...
		if (!ptn) {
			...
			return ptn.replace(me._re, function(a, b, c) {
						...
						if (!rel) {
							...
							b == "y" && (v %= 100);
							b == "M" && v++;
							return v < 10 ? QZFL.string.fillLength(v, 2, c) : v
									.toString()
						} else
							return Math.floor(Math.abs(date - baseTime)
									/ map[b][1])
					})
	};
	...
	(function() {
		...
		var Sizzle = function(selector, context, results, seed) {
			...
			var m, set, checkSet, extra, ret, cur, pop, i, prune = true, contextXML = Sizzle
					.isXML(context), parts = [], soFar = selector, speedUpMatch;
			if (!contextXML) {
					...
					if (context.nodeType === 1 || context.nodeType === 9)
						if (speedUpMatch[1])
							return makeArray(context
									...
									results);
						else if (speedUpMatch[2] || speedUpMatch[4]
								&& speedUpMatch[5])
							if (context.getElementsByClassName
									&& speedUpMatch[2])
								return makeArray(
										context
												.getElementsByClassName(speedUpMatch[2]),
										results);
							else {
								var suElems = context
										.getElementsByTagName(speedUpMatch[4]
												|| "*"), suResBuff = [], suIt, suCN = " "
										+ (speedUpMatch[2] || speedUpMatch[5])
										+ " ";
								for (var sui = 0, sulen = suElems.length; sui < sulen; ++sui) {
									suIt = suElems[sui];
									(" " + suIt.className + " ").indexOf(suCN) > -1
											&& suResBuff.push(suIt)
								}
								return makeArray(suResBuff, results)
							}
					if (context.nodeType === 9)
						if ((selector === "body" || selector.toLowerCase() === "body")
								&& context.body)
							return makeArray([context.body], results);
						else if (speedUpMatch[3])
							return (tmpVar = context
									.getElementById(speedUpMatch[3]))
									? makeArray([tmpVar], results)
									: makeArray([], results)
				}
			}
			do {
	...
	(function() {
		...
		_handler.prototype = {
			_init : function(selector, context) {
				if (QZFL.lang.isString(selector))
					...
					this.elements = selector;
						...
						&& (selector.nodeType && selector.nodeType !== 3
								&& selector.nodeType !== 8 || selector.setTimeout))
					this.elements = [selector];
				else
					this.elements = []
			},
			...
			filter : function(expr, elems, not) {
				if (not)
					expr = ":not(" + expr + ")";
				return _el.get(QZFL.selector.matches(expr, elems
								|| this.elements))
			},
			each : function(fn) {
			...
			concat : function(elements) {
				return _el.get(this.elements
						.concat(!!elements._isElementHandler
								? elements.elements
								: elements))
			},
			get : function(index) {
			...
			slice : function() {
				return _el.get(Array.prototype.slice.apply(this.elements,
						arguments))
			}
		};
	...
	QZFL.element.extendFn({
		...
		unBind : function(evtType, fn) {
			return this.each(function(el) {
						QZFL.event[fn ? "removeEvent" : "purgeEvent"](el,
								evtType, fn)
					})
		},
		...
		delegate : function(selector, eventType, fn, argsArray) {
			if (typeof fn != "function")
				return false;
			return this.each(function(el) {
						QZFL.event.delegate(el, selector, eventType, fn,
								argsArray)
					})
		},
	...
	QZFL.element.extendFn({
			...
			if (QZFL.object.getType(value) == "array") {
				...
				this.each(function(el) {
					if (/radio|checkbox/.test(el.type))
						el.checked = el.nodeType
								&& ("\x00"
										+ _v.indexOf(el.value.toString()
												+ "\x00") > -1 || _v
										.indexOf("\x00" + el.name.toString()
												+ "\x00") > -1);
					else if (el.tagName == "SELECT")
						QZFL.object.each(el.options, function(e) {
									e.selected = e.nodeType == 1
											&& ("\x00"
													+ _v.indexOf(e.value
															.toString()
															+ "\x00") > -1 || _v
		...
		show : function(isBlock) {
			return this.each(function(el) {
						QZFL.dom
								.setStyle(el, "display", isBlock ? "block" : "")
					})
		},
	...
	QZFL.element.extendFn({
		create : function(tagName, attributes) {
			var _arr = [];
			this.each(function(el) {
						_arr.push(QZFL.dom.createElementIn(tagName, el, false,
								attributes))
					});
			return QZFL.element.get(_arr)
		},
	...
	QZFL.effect = {
		...
		mode : [],
		init : function() {
			var classArray = [["webkit", "WebkitTransition"],
					["firefox", "MozTransition"], ["opera", "OTransition"],
					["ie", "msTransition"]], ua = QZFL.userAgent, agent = "", cName = "";
		...
		run : function(elem, prop, opts) {
			...
			if (o.off) {
				...
				var t = new QZFL.tweenMaker(0, 100, opts.duration,
						opts.interval, opts);
		...
		_prop : function(prop, elem) {
			...
			QZFL.object.each(prop, function(val, pname) {
						...
						if (QZFL.object.getType(val) == "object") {
							...
							endCSSMap[pname] = val.value = f.end
									+ (f.unit ? f.unit : 0);
							if (webkit)
								fprop[pname] = val;
							else
		...
		_cssValue : function(elem, val, name) {
			var fnum = /^([+\-]=)?([\d+.\-]+)([a-z%]*)$/i, fprop = {}, parts = fnum
					.exec(val + ""), o = QZFL.effect, start = o
					._cur(elem, name);
			if (parts) {
				var end = parseFloat(parts[2]), unit = parts[3]
						|| (o._cssNumber[name] ? "" : "px");
				if (unit !== "px") {
		...
		_cur : function(elem, p) {
			...
			if (elem != null && elem[p] != null
					&& (!elem.style || elem.style[p] == null))
				return elem[p];
		...
		_checkVerticalStyle : function(elem, opts) {
			var name = ["marginTop", "marginBottom", "paddingTop",
					"paddingBottom", "height"], obj = {}, D = QZFL.dom, re, o = QZFL.effect;
			if (opts.clear) {
	...
	QZFL.tweenMaker = function(startvalue, endvalue, duration, interval, opt) {
		...
		o.functor = opt.functor || o.functors[easing] || o.functors["ease"];
	...
	QZFL.tweenMaker.prototype = {
		...
		_playTimer : function(time) {
			var _end = false, o = this, time = (new Date).getTime()
					- o._startTime;
			if (time > o.duration) {
				time = o.duration;
				_end = true
			}
			o.currentValue = o.functor(time, o.startValue, o.changeValue,
					o.duration);
		...
		getPercent : function() {
			return (this.currentValue - this.startValue) / this.changeValue
					* 100
		}
	};
	QZFL.cssTransfrom = function(elem, prop, opts) {
		...
		if (!o._running && prop) {
			...
			o._count = 0;
			o._running = false;
			o._callback = QZFL.lang.isFunction(opts.complete)
					? opts.complete
					: QZFL.emptyFn;
	...
	QZFL.cssTransfrom.prototype = {
		init : function() {
			var map = [
					["webkit", "-webkit-transition", "WebkitTransition",
							"webkitTransitionEnd", "WebkitTransform"],
					["firefox", "-moz-transition", "MozTransition",
							"transitionend", "MozTransform"],
					["opera", "-o-transition", "OTransition", "oTransitionEnd",
							"OTransform"],
					["ie", "-ms-transition", "msTransition", "MSTransitionEnd",
							"msTransform"]], tiClassPrefix, tiStyleName, tiEvtName, tfStyleName, ua = QZFL.userAgent, proto;
		...
		firecss : function() {
			...
			if (!ct._hasEnd[uid]) {
				elem.addEventListener(this.TRANSITION.event,
						elem._transitionCb = function(e) {
			...
			elem.style.cssText += ["", this.pptKey + ":" + pptVal.join(","),
					...
					delayKey + ":" + delayVal.join(","), cssText, ""].join(";")
		},
		_toHyphen : function(prop) {
			prop = prop.replace(/([A-Z]?)([a-z]+)([A-Z]?)/g, function(m0, m1,
							m2, m3) {
						var str = "";
		...
		_endTransfrom : function(sname) {
			var QF = QZFL, elem = this._elem, pptKey = this.pptKey, value = QF.dom
					.getStyle(elem, pptKey);
			if (!value) {
		...
		_onTransfromEnd : function(e, uid) {
			var pname = QZFL.string.camelCase(e.propertyName), elapsed = e.elapsedTime, attrs = QZFL.cssTransfrom._attrs[uid], attr = attrs
					&& attrs[pname], tran = attr ? attr.transition : null, _cprop = "";
			if (tran) {
		...
		_addProperty : function(prop, conf) {
			...
			o._count++;
			dur = (typeof conf.duration != "undefined"
					? conf.duration
					: o._duration)
					|| 1.0E-4;
			attrs[prop] = {
				value : val,
				duration : dur,
				delay : typeof conf.delay != "undefined"
						? conf.delay
						: o._delay,
		...
		clearStatus : function(elem, style) {
			...
			if (elem._transitionCb) {
				elem.removeEventListener(this.TRANSITION.event,
						elem._transitionCb, false);
	...
	QZFL.Tween.prototype.start = function() {
		...
		if (set) {
			...
			o.opts.change = function(p) {
				...
				o.onMotionChange
						&& o.onMotionChange.apply(o, [o.elem, o.pname, v])
			};
			o.onMotionStart.apply(o);
			QZFL.effect.run(o.elem, o.prop, o.opts)
		} else {
			o.css = false;
			var t = new QZFL.tweenMaker(s, e, o.opts.duration, o.opts.interval
							|| 15);
			t.onStart = function() {
				o.t = t;
				o.onMotionStart.apply(o)
	...
	QZFL.Tween.prototype.getPercent = function() {
		return this.css ? QZFL.effect.getPercent(this.elem) : this.t
				.getPercent()
	};
	...
	(function() {
		var _easeAnimate = function(_t, a1, a2, ease) {
			var _s = QZFL.dom["get" + _t](this), _reset = typeof a1 != "number"
					&& typeof a2 != "number";
			if (_t == "Size" && _reset) {
			...
			var _v1 = _s[0] - a1;
			var _v2 = _s[1] - a2;
			var n = new QZFL.Tween(this, "_p", QZFL.transitions[ease]
							|| QZFL.transitions.regularEaseOut, 0, 100, 0.5);
			n.onMotionChange = QZFL.event.bind(this, function() {
						var _p = arguments[2];
						QZFL.dom["set" + _t](this, typeof a1 != "number"
										? _s[0]
										: _s[0] - _p / 100 * _v1,
								typeof a2 != "number" ? _s[1] : _s[1] - _p
										/ 100 * _v2)
					});
			if (_t == "Size" && _reset)
		...
		var _easeShowAnimate = function(_t, ease) {
			var n = new QZFL.Tween(this, "opacity", QZFL.transitions[ease]
							|| QZFL.transitions.regularEaseOut, _t ? 0 : 1, _t
							? 1
							: 0, 0.5);
		...
		var _easeScroll = function(top, left, ease) {
			if (this.nodeType == 9)
				var _stl = [QZFL.dom.getScrollTop(this),
			...
			var n = new QZFL.Tween(this, "_p", QZFL.transitions[ease]
							|| QZFL.transitions.regularEaseOut, 0, 100, 0.5);
			n.onMotionChange = QZFL.event.bind(this, function() {
				var _p = arguments[2], _t = _stl[0] - _p / 100 * _st, _l = _stl[1]
						- _p / 100 * _sl;
				if (this.nodeType == 9) {
		...
		QZFL.element.extendFn({
					...
					effectShow : function(effect, ease) {
						...
						if (effect == "resize")
							this.each(function(el) {
										_easeAnimate.apply(el, ["Size", null,
														null, ease])
									})
					},
					effectHide : function(effect, ease) {
						...
						if (effect == "resize")
							this.each(function(el) {
										_easeAnimate.apply(el, ["Size", 0, 0,
														ease])
									})
					},
					effectResize : function(width, height, ease) {
						this.each(function(el) {
									_easeAnimate.apply(el, ["Size", width,
													height, ease])
								})
					},
	...
	QZFL.Deferred = function(func, args) {
		...
		QZFL.object.extend(Promise.prototype, {
					...
					then : function(doneFunc, failFunc) {
						return this.add("resolve", doneFunc).add("reject",
								failFunc)
					},
					resolve : function() {
	...
	QZFL.object.extend(QZFL, {
		when : function(promise) {
			var _slice = Array.prototype.slice, promises = _slice
					.call(arguments), length = promises.length, remain, updateFunc, datas = [], d, s;
	...
	QZFL.XHR = function(actionURL, cname, method, data, isAsync, nocache) {
		...
		prot._name = cname;
		prot._nc = !!nocache;
		prot._method = (typeof method == "string" ? method : "").toUpperCase() != "GET"
				? "POST"
				: "GET";
	...
	QZFL.XHR.instance = {};
	QZFL.XHR.counter = 0;
	QZFL.XHR.path = "http://" + QZFL.config.resourceDomain
			+ "/ac/qzfl/release/expand/xhr_base.js?max_age=864001", QZFL.XHR.prototype.send = function() {
		var _s = QZFL.XHR, fn;
		...
		fn = location.host && this._uriObj.host != location.host
				? "_DoXsend"
				: "_DoSend";
		if (_s[fn])
			return _s[fn](this);
	...
	QZFL.XHR.genHttpParamString = function(o, cs) {
		...
		for (var i in o)
			r.push(i
					+ "="
					+ (cs == "utf-8" ? encodeURIComponent(o[i]) : QZFL.string
							.URIencode(o[i])));
		return r.join("&")
	};
	(function() {
		...
		}, extend = QZFL.object.extend, cdm = location.host, supportlv2 = (QZFL.userAgent.ie > 9 || window.XMLHttpRequest)
		...
		function processText(str) {
			var lidx = str.lastIndexOf(")"), rst = str.substring(str
							.indexOf("(")
							+ 1, lidx == -1 ? str.length : lidx);
		...
		function ajaxRequest(url, data, opt) {
			...
			if (isSameDomain(url)) {
				...
				xhr.onreadystatechange = function() {
					if (this.readyState == 4)
						if (this.status >= 200 && this.status < 300) {
							opt.onload.call(this, eval("("
											+ processText(this.responseText)
											+ ")"));
							QZFL["FormSender"]._pluginsRunner(
									"onRequestComplete", opt.instance)
						}
				}
			} else if (supportlv2) {
				xhr.open(opt.method, url + (url.indexOf("?") > -1 ? "&" : "?")
				...
				xhr.onload = function() {
					opt.onload.call(this, eval("("
									+ processText(this.responseText) + ")"));
					QZFL["FormSender"]._pluginsRunner("onRequestComplete",
							opt.instance)
				}
			} else {
				xhr = new QZFL.FormSender(url, "post", data);
				xhr.onSuccess = opt.onSuccess;
		...
		QZFL.ajaxPost = function(url, data, opt) {
			opt = extend(def, typeof opt == "function" ? {
						onload : opt
					} : opt || {});
			ajaxRequest(url, data, extend(opt, {
								method : "post"
							}))
		};
	...
	QZFL.cookie = {
		...
		get : function(name) {
			var r = new RegExp("(?:^|;+|\\s+)" + name + "=([^;]*)"), m = document.cookie
					.match(r);
			return !m ? "" : m[1]
		},
	...
	QZFL.debug = {
		...
		startDebug : function() {
			window.onerror = function(msg, url, line) {
				...
				QZFL.console.print(msg + "<br/>" + urls[urls.length - 1]
								+ " (line:" + line + ")", 1);
		...
		showLog : function() {
			...
			if (!!o)
				o.innerHTML = QZFL.string.nl2br(QZFL.string
						.escHTML(this.errorLogs.join("\n")))
		},
		...
		function stack(e, a) {
			function genTrace(ee, aa) {
				if (ee.stack)
					...
					var fname = fnp.test(entry.toString()) ? fnp.exec(entry
							.toString())[1] : "<ANON>";
	...
	QZFL.JsLoader.prototype._load = function() {
		...
		return function(src, doc, opts) {
			var o = this, tmp, k, head = doc.head
					|| doc.getElementsByTagName("head")[0] || doc.body, _new = false, _js;
			if (!(_js = doc.getElementById(idp.pop())) || QZFL.userAgent.ie
					&& QZFL.userAgent.ie > 8) {
			...
			_ae(_js, _ie && _ie < 10 ? "readystatechange" : "load", function() {
						if (!_js
								...
								&& (typeof docMode == "undefined"
										|| docMode < 10
										? _js.readyState != "loaded"
										: _js.readyState != "complete"))
							return;
						_ie && idp.push(_js.id);
						o.onload();
						!_ie && _rm(_js);
	...
	QZFL.imports = function(sources, succCallback, opts) {
		...
		var errCallback = isFn(opts.errCallback)
				? opts.errCallback
				: QZFL.emptyFn;
	...
	QZFL.imports.getUrl = function(url) {
		return QZFL.string.isURL(url) ? url : QZFL.imports._indirectUrlRE
				.test(url) ? url : QZFL.config.staticServer + url + ".js"
	};
	...
	QZFL.FormSender = function(actionURL, method, data, charset) {
		...
		if (typeof actionURL == "object" && actionURL.nodeType == 1
				&& actionURL.tagName == "FORM")
			this.instanceForm = actionURL;
		else {
			this.method = method || "POST";
			this.uri = actionURL;
			this.charset = c == "utf-8" || c == "gbk" || c == "gb2312"
					|| c == "gb18030" ? c : "gb2312";
			this.data = typeof data == "object" || typeof data == "string"
					? data
					: null;
			this.proxyURL = this.charset == "utf-8" ? QZFL.config.FSHelperPage
					.replace(/_gbk/, "_utf8") : QZFL.config.FSHelperPage
		}
		this._sender = null;
		this.onSuccess = QZFL.emptyFn;
		this.onError = QZFL.emptyFn;
	...
	QZFL.FormSender._clear = function(o) {
		...
		if (QZFL.userAgent.safari || QZFL.userAgent.opera)
			setTimeout(
					'QZFL.dom.removeElement(document.getElementById("_fp_frm_'
							+ o.name + '"))', 50);
		else
			QZFL.dom
					.removeElement(document.getElementById("_fp_frm_" + o.name));
		o.endTime = +new Date;
		QZFL.FormSender._pluginsRunner("onRequestComplete", o);
		o.instanceForm = null
	};
	QZFL.FormSender.prototype.send = QZFL.config.xhrProxyEnable == 1
			&& QZFL.cookie.get("speedup") == "dynamic" ? function() {
		QZFL.ajaxPost(QZFL.config.xhrProxy, this.data, {
					headers : {
						"X-Real-Url" : this.uri
								+ (this.uri.indexOf("?") == -1 ? "?" : "&")
								+ "g_tk=" + QZFL.pluginsDefine.getACSRFToken()
		...
		if (QZFL.config.canUseXHR2 && QZFL.config.canUseXHR2(this.uri))
			return QZFL.ajaxPost(this.uri, this.data, {
						onload : this.onSuccess,
						instance : this
					});
		...
		if (!this.instanceForm) {
			...
			ifrHTML = '<!DOCTYPE html><html lang="zh-cn"><head><meta http-equiv="content-type" content="text/html; charset='
					+ t.charset + '" /><meta charset="' + t.charset + '" />';
			if (ie)
				ifrurl = 'javascript:document.open();document.domain="'
						+ document.domain
						+ '";var me=parent.QZFL.FormSender.instance["' + t.name
						+ '"];document.write(me.ifrHTML);document.close();';
			ifrHTML = ifrHTML
					+ '<script type="text/javascript">'
					+ (ie && 'document.charset="' + t.charset + '"' || "")
					+ ';document.domain="'
					+ document.domain
					+ '";frameElement.submited=void(0);frameElement.state="sending";<\/script></head><body>';
			ifrHTML = ifrHTML
					+ '<form action="'
					+ t.uri
					+ (t.uri.indexOf("?") > -1 ? "&" : "?")
					...
					+ '" accept-charset="'
					+ t.charset
					+ '" id="p" enctype="application/x-www-form-urlencoded;charset='
					+ t.charset + '" method="post">';
			ifrHTML = ifrHTML
					+ '<input type="hidden" name="qzreferrer" id="qzreferrer" />';
			ifrHTML = ifrHTML
					+ '</form><script type="text/javascript">var me=parent.QZFL.FormSender.instance["'
					+ t.name
	...
	QZFL.JSONGetter = function(actionURL, cname, data, charset, junctionMode) {
		...
		prot._uri = actionURL;
		prot._data = data
				&& (QZFL.object.getType(data) == "object" || QZFL.object
						.getType(data) == "string") ? data : null;
		prot._charset = QZFL.object.getType(charset) != "string"
				? QZFL.config.defaultDataCharacterSet
				: charset;
	...
	QZFL.JSONGetter.prototype.send = function(callbackFnName) {
		if (this._waiting)
			return;
		var clear, cfn = QZFL.object.getType(callbackFnName) != "string"
				? "callback"
				: callbackFnName, da = this._uri;
		if (this._data)
			da += (da.indexOf("?") < 0 ? "?" : "&")
					+ (typeof this._data == "object" ? QZFL.JSONGetter
							.genHttpParamString(this._data) : this._data);
		...
		if (QZFL.userAgent.ie
				&& QZFL.userAgent.ie < 10
				&& !(QZFL.userAgent.beta && navigator.appVersion
						.indexOf("Trident/4.0") > -1)) {
			var df = document.createDocumentFragment(), sender = (QZFL.userAgent.ie > 8
					? document
					: df).createElement("script");
			...
			var callback = function(th) {
				return function() {
					...
					QZFL.JSONGetter._runFnQueue(th._squeue,
							QZFL.JSONGetter.args, th);
			...
			node.src = da;
			baseElement ? h.insertBefore(node, baseElement) : h
					.appendChild(node)
		}
	};
			...
			var opts = {
				...
				callback : function(cb) {
					return function(o, type, ew, eh, p) {
						QZFL.lang.isFunction(cb)
								&& cb(o, ew, eh, null, p.ow, p.oh, p)
					}
				}(callback),
		...
		getFlashHtml : function(flashArguments, requiredVersion, flashPlayerCID) {
			...
			for (var k in flashArguments)
				switch (k) {
					...
					case "src" :
						if (QZFL.userAgent.ie)
							_params.push('<param name="movie" value="',
									flashArguments.noSrc
											? ""
											: flashArguments[k], '"/>');
						else
							_attrs.push(' data="', flashArguments.noSrc
											? ""
											: flashArguments[k], '"');
						break;
					default :
						_params.push('<param name="', k, '" value="',
								flashArguments[k], '" />')
				}
			if (QZFL.userAgent.ie)
				_attrs.push(' classid="clsid:', flashPlayerCID
								|| "D27CDB6E-AE6D-11cf-96B8-444553540000", '"');
			else
				_attrs.push(' type="application/x-shockwave-flash"');
			if (requiredVersion
					&& requiredVersion instanceof QZFL.media.SWFVersion) {
			...
			return "<object" + _attrs.join("") + ">" + _params.join("")
					+ "</object>"
		},
		insertFlash : function(containerElement, flashArguments) {
			if (!containerElement
					|| typeof containerElement.innerHTML == "undefined")
				return false;
			...
			containerElement.innerHTML = QZFL.media
					.getFlashHtml(flashArguments);
		...
		getWMMHtml : function(wmpArguments, cid) {
			...
			for (var k in wmpArguments)
				switch (k) {
					...
					default :
						objArgm.push(" ", k, '="', wmpArguments[k], '"');
						params.push('<param name="', k, '" value="',
								wmpArguments[k], '" />')
				}
			if (wmpArguments["src"])
	...
	QZFL.media.SWFVersion.prototype.toNumber = function() {
		var se = 0.0010;
		return this.major + this.minor * se + this.rev * se * se + this.add
				* se * se * se
	};
	QZFL.media.getFlashVersion = function() {
		if (!QZFL.media._flashVersion) {
			...
			if (navigator.plugins && navigator.mimeTypes.length) {
				...
				try {
					resv = new QZFL.media.SWFVersion(axo
	...
	QZFL.media.reduceImage = function() {
		var doReduce = function(o, type, ew, eh, p, cb) {
			...
			type ? (rl > p.shortSize ? rl = p.shortSize : 1)
					&& (p.k = p.shortSize / rl) : (rl > p.longSize
					? rl = p.longSize
					: 1)
					&& (p.k = p.longSize / rl);
		...
		return function(type, ew, eh, opts) {
			opts = opts || {};
			opts.img = QZFL.lang.isNode(opts.img) ? opts.img : QZFL.event
					.getTarget();
			...
			if (opts.img)
				if (!(opts.direction && opts.rate && opts.longSize && opts.shortSize))
					r = QZFL.media.getImageInfo(function(o, p) {
								if (!o || !p)
									return;
								doReduce(opts.img, type, ew, eh, p,
										opts.callback)
							}, opts);
				else
	...
	QZFL.media.imagePlusUrl = "http://" + QZFL.config.resourceDomain
			+ "/ac/qzfl/release/widget/smart_image.js?max_age=1209603";
	QZFL.media.smartImage = function(w, h, params) {
		params = params || {};
		params.img = QZFL.lang.isNode(params.img) ? params.img : QZFL.event
				.getTarget();
	...
	QZFL.shareObject.getValidSO = function() {
		...
		for (var i = 1; i < cnt + 1; ++i)
			if (QZFL.shareObject.instance["so_" + i]
					&& QZFL.shareObject.instance["so_" + i]._ready)
	...
	QZFL.dragdrop = {
		path : "http://" + QZFL.config.resourceDomain
				+ "/ac/qzfl/release/widget/dragdrop.js?max_age=864001",
		dragdropPool : {},
		count : 0,
		registerDragdropHandler : function(handler, target, opts) {
			var _e = QZFL.event, _s = QZFL.dragdrop, _hDom = QZFL.dom
					.get(handler), _tDom = QZFL.dom.get(target), targetObject;
			opts = opts || {
			...
			_e.on(_hDom, "mousedown", _s.startDrag, [_hDom.id, targetObject,
							opts]);
		...
		unRegisterDragdropHandler : function(handler) {
			...
			if (!_hDom)
				return null;
			_hDom.style.cursor = "";
			QZFL.dragdrop._oldSD
					&& _e.removeEvent(_hDom, "mousedown", QZFL.dragdrop._oldSD);
	...
	QZFL.element.extendFn({
				dragdrop : function(target, opts) {
					var _arr = [];
					this.each(function() {
								_arr.push(QZFL.dragdrop
										.registerDragdropHandler(this, target,
												opts))
							});
					return _arr
				},
				unDragdrop : function(target, opts) {
					this.each(function() {
								_arr.push(QZFL.dragdrop
										.unRegisterDragdropHandler(this))
							})
				}
			});
	QZFL.widget.msgbox = {
		cssPath : "http://" + QZFL.config.resourceDomain
				+ "/ac/qzfl/release/css/msgbox.css",
		path : "http://" + QZFL.config.resourceDomain
				+ "/ac/qzfl/release/widget/msgbox.js",
		currentCssPath : null,
		_loadCss : function(s) {
	...
	QZFL.dialog = {
		cssPath : "http://" + QZFL.config.resourceDomain
				+ "/qzone_v6/qz_dialog.css",
		currentCssPath : "",
		path : "http://" + QZFL.config.resourceDomain
				+ "/ac/qzfl/release/widget/dialog.js?max_age=864010",
		count : 0,
		instances : {},
		BUTTON_TYPE : {
			Disabled : -1,
		...
		create : function(title, content, opts) {
			...
			if (t = typeof opts != "number" || isNaN(parseInt(opts, 10))) {
				opts = opts || {};
				args = [0, 0, opts.width, opts.height, opts.useTween,
						opts.noBorder]
			} else {
				opts = {
					"width" : opts
	...
	})(["hide", "unload", "init", "fillTitle", "fillContent", "setSize",
			"show", "hide", "focus", "blur", "setReturnValue"]);
	...
	QZFL.widget.Confirm.cssPath = "http://" + QZFL.config.resourceDomain
			+ "/ac/qzfl/release/resource/css/confirm_by_dialog.css";
	QZFL.widget.Confirm.path = "http://" + QZFL.config.resourceDomain
			+ "/ac/qzfl/release/widget/confirm_base.js";
	...
	QZFL.maskLayout = function() {
		var masker = null, count = 0, qml = function(zi, doc, opts) {
			...
			masker.style.cssText = "background-color:#000;filter:alpha(opacity="
					+ 100
					* opts.opacity
					+ ");opacity:"
					+ opts.opacity
					...
					+ zi
					+ ";width:100%;height:"
					+ QZFL.dom[QZFL.userAgent.ie < 7
							? "getScrollHeight"
							: "getClientHeight"](doc) + "px;";
			return count
		};
	...
	QZFL.fixLayout = {
		...
		_init : function() {
			...
			this._isInit = true;
			if (this._isIE6)
				QZFL.event.addEvent(document.compatMode == "CSS1Compat"
								? window
								: document.body, "scroll", QZFL.event.bind(
								this, this._onscroll))
		},
		create : function(html, isBottom, layerId, noFixed, options) {
			...
			var tmp = {
				style : (isBottom ? "bottom:0;" : "top:0;")
						+ (options.style || "left:0;width:100%;z-index:10000")
			}, _c;
			if (layerId)
				tmp.id = layerId;
			this._layoutCount++;
			_c = this._layoutDiv[this._layoutCount] = QZFL.dom.createElementIn(
					"div", this._fixLayout, false, tmp);
		...
		_doScroll : function() {
			...
			for (var k in o._layoutDiv) {
				...
				if (_item.isTop)
					o._layoutDiv[k].style.marginTop = QZFL.dom.getScrollTop()
							+ "px";
				else {
	...
	QZFL.widget.bubble = {
		show : function(target, title, msg, opts) {
			...
			QZFL.imports(QZFL.widget.bubble.path, function() {
						QZFL.widget.tips.show("<div>" + title + "</div>" + msg,
								target, opts)
					});
			return bid
		},
	...
	QZFL.widget.tips = {
		path : "http://" + QZFL.config.resourceDomain
				+ "/ac/qzfl/release/widget/tips.js?max_age=1209600",
		...
		resize : function(id) {
			QZFL.imports(QZFL.widget.tips.path, function() {
						if (QZFL.widget.tips)
							QZFL.widget.tips.resize
									&& QZFL.widget.tips.resize(id)
					})
		}
	};
	...
	QZFL.widget.seed = {
		...
		update : function(k, opt) {
			...
			if (typeof k == "undefined")
				...
				if (opt && opt.useCookie) {
					n = QZFL.cookie.get(k);
					if (n)
						QZFL.cookie.set(k, ++n, opt.domain || th.domain, null,
								3E3);
					else
						return 0
				} else {
		...
		_update : function() {
			...
			QZFL.cookie.set("randomSeed", th._seed = parseInt(Math.random()
									* 1E6, 10), th.domain, null, 3E3);
			return th._seed
		},
		get : function(k, opt) {
			var s, n, th = QZFL.widget.seed;
			if (typeof k == "undefined")
				return (th._seed = QZFL.cookie.get("randomSeed"))
						? th._seed
						: th.update();
			else {
				k = th.prefix + k;
				if (opt && opt.useCookie)
					return (n = QZFL.cookie.get(k)) ? n : (QZFL.cookie.set(k,
							n = 1, opt.domain || th.domain, null, 3E3), n);
				else {
	...
	QZFL.widget.runBox = function(startNode, endNode, opts) {
		...
		if (!QZFL.lang.isNode(startNode) || !QZFL.lang.isNode(endNode)
				|| !QZFL.effect)
			return;
		opts = opts || {};
		...
		dv.style.cssText = "border:3px solid #999; z-index:10000; position:absolute; left:"
				+ sp.left
				+ "px; top:"
				+ sp.top
				+ "px; width:"
				+ sp.width
				+ "px; height:" + sp.height + "px;";
	...
	if (!QZFL.pluginsDefine)
		QZFL.pluginsDefine = {};
	QZFL.pluginsDefine.networkChectLibPath = "http://"
			+ QZFL.config.resourceDomain
			+ "/qzone/v6/troubleshooter/network_check_plugin_lib.js";
	(function() {
		var t = QZONE.FormSender;
		if (t && t.pluginsPool)
			t.pluginsPool.onErrorHandler.push(function(fsObj) {
				fsObj
						...
						&& QZFL.imports
						&& QZFL.imports(QZFL.pluginsDefine.networkChectLibPath,
								function(d) {
									return function() {
										QZONE
	...
	(function(q) {
		var commurl = "http://c.isdspeed.qq.com/code.cgi", urlParse = /^http:\/\/([\s\S]*?)(\/[\s\S]*?)(?:\?|$)/, pingSender = q.pingSender, collector = [], timer, isreportG = Math
				.random()
				* 2E3 < 1, isreportP = Math.random() * 100 < 1, uin = typeof g_iUin == "undefined"
				? 0
				: g_iUin, duration = 1E3, each = q.object.each;
		function valueStat(domain, cgi, type, code, time, rate, uin, exts) {
			if (Math.random() > 1 / rate)
				return;
			var param = [];
			param.push("uin=" + uin, "key=" + "domain,cgi,type,code,time,rate",
					"r=" + Math.random());
			...
			if (domain != "" || i > 0)
				q.pingSender
						&& q.pingSender(commurl + "?" + param.join("&"), 1E3)
		}
		function _r() {
			if (collector.length)
			...
			if (q[n] && q[n].pluginsPool) {
				...
				q[n].pluginsPool.onRequestComplete.push(function(th) {
					...
					if (th.msg && th.msg.indexOf("Connection") > -1) {
						collector.push([domain, url, 2, 502,
								+th.endTime - th.startTime, 1]);
						return
					}
					var d = th.resultArgs;
					if (d && (d = d[0]))
						if (typeof d.code == "undefined")
							...
							collector.push([domain, url, 3, d.subcode || 1,
									+th.endTime - th.startTime, 1]);
						else {
							if (th instanceof q.JSONGetter)
								if (th.reportRate)
									(th.reportRate == 1 || Math.random() < 1
											/ th.reportRate)
											&& collector.push([domain, url, 1,
													d.subcode || 1,
													+th.endTime - th.startTime,
													th.reportRate || 2E3]);
								else
									isreportG
											&& collector.push([domain, url, 1,
				...
				q[n].pluginsPool.onRequestComplete.push(function(th) {
					if (th instanceof q.FormSender) {
						var stat_url = "http://isdspeed.qq.com/cgi-bin/r.cgi?flag1=175&flag2=363&flag3=51", takes = th.postTime
								- th.startTime, s;
						if (ua.ie == 8)
							s = "2";
						else if (ua.ie == 9)
...
if (QZFL.userAgent.ie)
	eval((QZFL.userAgent.ie < 9 ? "var document = QZFL._doc;" : "")
			+ "var setTimeout = QZFL._setTimeout, setInterval = QZFL._setInterval");

try {
	(function(_w) {
		_w._javascript_file_map = _w._javascript_file_map || {};
		_w._javascript_file_map['ac/qzone/qzfl/qzfl_2.1.29.2.js'] = true;
	})(window);
} catch (ign) {
}
/*  |xGv00|6f225fa9b4ead11dca8a47f4ad640d52 */(function() {
	QZFL.config = QZFL.config || {};
	var list = [['taotao.qq.com', 'emotion_cgi_publish_v6'],
			['taotao.qq.com', 'emotion_cgi_re_feeds'],
			['taotao.qq.com', 'emotion_cgi_addcomment_ugc'],
			['taotao.qq.com', 'emotion_cgi_addreply_ugc'], ''].slice(0, -1)
	var re = /^https?\:\/\/([\s\S]+?)\//, rst;
	var supportlv2 = (QZFL.userAgent.ie > 9 || window.XMLHttpRequest)
			&& ('withCredentials' in new XMLHttpRequest);

	QZFL.config.xhrProxyEnable = function(url) {
		if (location.href.indexOf('proxydebug') > -1)
			return 1;
		return url.indexOf('taotao.qq.com') > -1;
	};
	QZFL.config.canUseXHR2 = function(url) {
		if (!supportlv2)
			return false;
		rst = url.match(re);
		for (var i = 0; i < list.length; i++) {
			if (list[i][0] == rst[1]
					&& new RegExp("\u005C/" + list[i][1] + "\u005Cb").test(url)) {
				return true;
			}
		}
		return false;
