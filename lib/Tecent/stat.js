			...
			if (items.length) {
				...
				if (i > 0) {
					noDelay ? pvSender.doSend() : (timer = setTimeout(
							pvSender.doSend, (typeof timeout == 'undefined'
									? 5000
									: timeout)));
				}
			}
...
TCISD.valueStat.send = function(statId, resultType, returnValue, opts) {
	...
	url.push("flag1=", statId, "&", "flag2=", resultType, "&", "flag3=",
			returnValue, "&", "1=", (TCISD.valueStat.config.reportAll
					? 1
...
TCISD.hotClick._reportRate = typeof TCISD.hotClick._reportRate == 'undefined'
		? -1
...
TCISD.stringStat.genHttpParamString = function(o) {
	...
	return res.join('&');
};/*  |xGv00|f15b460ad39cdea2d150a2cb31f1553c */